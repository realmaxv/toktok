...
export default function Shuffle() {
  ...
  useEffect(() => {
    const updateGridConfig = () => {
      ...
      const rowsNeeded = Math.ceil(
        (targetHeight / cellHeight) * maxRowsPerItem
      ); // 需要的行数（保守估计）
      const cellsNeeded = rowsNeeded * config.columns; // 需要的总单元格数
      setTotalCells(cellsNeeded);
      console.log(
        `Total cells needed: ${cellsNeeded}, Columns: ${config.columns}`
      );
    };
  ...
  const generateGridItems = useCallback(
    (existingItems: PostPreview[] = [], cellsToAdd: number) => {
      ...
      let occupiedCells = existingItems.reduce(
        (sum, item) => sum + item.rows * item.cols,
        0
      );
      ...
      const colorBlockCount = Math.floor(
        cellsToAdd * 0.1 + Math.random() * (cellsToAdd * 0.1)
      );
      const imageCount = Math.min(data.length, cellsToAdd - colorBlockCount);
      // 过滤掉已使用的图片
      const availableData = data.filter(
        (item) => !usedItemIds.current.has(item.id)
      );
      ...
      for (const item of validData) {
        ...
        // 过滤尺寸，确保不超过网格列数
        const filteredSizes = GRID_SIZES.filter(
          (size) => size.cols <= gridConfig.columns
        );
        let size =
          filteredSizes[Math.floor(Math.random() * filteredSizes.length)];
        if (size.rows === 3 && size.cols === 3 && count3x3 >= max3x3) {
          size = filteredSizes.find((s) => s.rows !== 3 || s.cols !== 3) || {
            rows: 1,
            cols: 1,
          };
        }
        if (size.rows === 2 && size.cols === 2 && count2x2 >= max2x2) {
          size = filteredSizes.find((s) => s.rows !== 2 || s.cols !== 2) || {
            rows: 1,
            cols: 1,
          };
        }
      ...
      while (occupiedCells < targetCells) {
        ...
        const cellsNeeded = size.rows * size.cols;
        if (
          occupiedCells + cellsNeeded <= targetCells &&
  ...
  }, [loadMore, isLoading, gridItems.length]);
